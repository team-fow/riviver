...
func do_tutorial(idx: int) -> void:
	...
	match idx:
		...
		2:
			...
			scienceguy.set_sprite(scienceguy.Sprite.HAPPY)
			await scienceguy.set_text("Click on the factory to investigate!")
			scienceguy.hide()
			...
			scienceguy.set_sprite(scienceguy.Sprite.HAPPY)
			await scienceguy.set_text("You can drag pipes onto the dirt grid.")
		...
		3:
			...
			scienceguy.set_sprite(scienceguy.Sprite.FRUSTRATED)
			await scienceguy.set_text("This factory is leaking bad chemicals into the water!")
			scienceguy.set_sprite(scienceguy.Sprite.ANGRY)
			await scienceguy.set_text("All those dirty pollutants will make our water dirty too!")
			scienceguy.set_sprite(scienceguy.Sprite.HAPPY)
			await scienceguy.set_text("Try using a filter! Filters clean water.")
		4:
			scienceguy.set_sprite(scienceguy.Sprite.FRUSTRATED)
			await scienceguy.set_text("What now?")
			scienceguy.set_sprite(scienceguy.Sprite.ANGRY)
			...
			await scienceguy.set_text("Click on a sandy riverbank to investigate.")
		...
		5:
			scienceguy.set_sprite(scienceguy.Sprite.HAPPY)
			await scienceguy.set_text("You've done a great job so far!")
			await scienceguy.set_text("The world is looking much prettier, wouldn't you agree?")
			scienceguy.set_sprite(scienceguy.Sprite.FRUSTRATED)
			await scienceguy.set_text("This is the final level, so do a good job!")
			await scienceguy.set_text("It's harder, but you can do it!")
