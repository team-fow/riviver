...
func play(target: Tile):
	if targets_water:
		...
		Game.grid.radiate_effect(target, Tile.set_type.bind(Tile.Type.RIVERBED), _can_erode, radius ** 2, true)
...
func can_be_played(target: Tile) -> bool:
	if targets_water:
		...
		return _can_erode(target) and target.get_neighbors().any(Tile.has_tag.bind("river"))
